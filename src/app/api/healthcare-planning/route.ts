...
export async function GET() {
  ...
  // Feature gate — healthcare_planning requires PRO or ADVISOR plan
  try {
    await requireFeature(session.user.id, 'healthcare_planning');
  } catch (err) {
    if (err instanceof Error && err.message === 'FEATURE_GATED') {
      return NextResponse.json(
        { error: 'Healthcare planning requires a Pro subscription.', upgradeRequired: true, upgradeUrl: '/app/settings/billing' },
        { status: 402 }
      );
    }
    throw err;
